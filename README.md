...
released python special edition https://github.com/dreadwrr/Developer-buddy/tree/pyver<br>
...
script includes recentchanges as well as save-changesnew. There is the bash (with python backend) which is the main release. A python version which is recentchanges implemented in python that I use for bridging from bash to qt. This streamlines development as new changes are made for primary releases. Both bash and qt being on the opposite sides of implementation.
...
also a shortcut rnt to inverse. recentchanges normally doesnt apply the filter but now it will. for a newer than file search with rnt search myfile it will remove the filter as that is normally filtered.<br><br>
...
high res icon https://drive.google.com/file/d/1ipiH5Spl9DEwVvcJ6smQixG9A7s2Qksy/view?usp=sharing <br>
Manual <br>
...
To save to backup in changes=EXIT:/ <br> isolateBACK is by default true. so 'save-changesnew backup' will write to the backup. this is in /changes.bak beside /changes
