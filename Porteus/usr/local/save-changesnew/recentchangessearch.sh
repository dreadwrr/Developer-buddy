#!/bin/bash
#
#      recentchanges search             Developer Buddy v3.0    08/11/2025
#
...
if [ "$2" != "noarguser" ] && [ "$2" != "" ]; then
	...
	if [ "$2" -ge 0 ] 2>/dev/null; then
    ...
    else # Its a newerthan file search
    	...
    	#cp $COMPLETE $RECENT	#we need two files one to work with and one to add customized time for user reading
                            	#due to output supression above
    fi
...
# The search is for system files
if [ "$tmn" != "" ]; then
	logf=$RECENT 
...
if [ -s $TMPCOMPLETE ]; then
	...
	# $tout is merged into $SORTCOMPLETE after main loop below
fi
rm $xdata
...
if [ "$mMODE" == "normal" ]; then
	...
	#xargs -0 -I{} -P4 /usr/local/save-changesnew/mainloop "{}" $atmp $checkSUM < $FEEDFILE      No improvement past 4
...
if [ -s $SORTCOMPLETE ]; then
	...
	if [ -s $tout ]; then grep -v 'NOTA-FI-LE 77:77:77' "$tout" | awk -v tme="$PRD" '{ ts = $1 " " $2; if (ts >= tme) print }' > $TMPOPT ; cat $TMPOPT >> $SORTCOMPLETE; fi   
	 ...
	 #   We dont want anything before our main files.
	#
	##
