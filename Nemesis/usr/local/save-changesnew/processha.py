...
from recentchangessearch import parse_datetime
...
# postprocess
def isdiff(RECENT, ABSENT, rout, diffnm, difff_file, parsed_PRD, fmt):
    ...
    for line in difff_file:
        ...
        if timestamp is None or timestamp >= parsed_PRD:
            continue
    ...
    with open(diffnm, 'a') as f:
        f.writelines(difff_file)
        f.write('\n')
        if ABSENT:
            print('Applicable to your search', f)
        else:

            print('None of above is applicable to search. It is the previous search', f)


# output
def processha(rout, ABSENT, diffnm, cerr, TMPOPT, flsrh, lclmodule, argf, parsed_PRD, USR, supbrwr, supress):
    ...
    if flsrh:
        ...
        DIFFMATCHED = [line for line in TMPOPT if parse_timestamp(line) >= parsed_PRD]
