...
deepscanBK=30   # Default 60    Change these for foward and back
deepscanFWR=60 # 60
scanBK=30 # 30
scanFWR=60 # 60
...
mkdir $tmp ; echo ; echo  "processing..."
if [ "$xMODE" == "compile" ]; then
    ...
    comp $f
    cyan    "searching back to get the delta" ; echo ; cyan "while this completes. Suggest grab a coffee" ; cyan "searching for files" $f" seconds old or newer"
	fnd $COMPLETE $qtn
else
    echo not intended use of compile feature. exiting ; exit
fi
...
comp $c
echo "please wait $qtn minutes. continue to use your computer like you normally would." ; echo ; echo "modify this script for a custom time"
totalTIME=$c ; sleep $c
fnd $COMPLETETWO $qtn
...
if [[ "$inputSED" == "true" ]]; then
	...
	sed -i -e '$d' $SORTCOMPLETE
	if [[ "$inputTIME" == "true" ]]; then awk '{print $3}' $SORTCOMPLETE > $TMPCOMPLETE ; else awk '{print $1}' $SORTCOMPLETE > $TMPCOMPLETE ; fi
   	sed -i -e 's![^/]*$!!' -e 's/\/$//' $TMPCOMPLETE ; fixcmp < "$TMPCOMPLETE" | fixOT > "$PRFLFLT" ; sed -i -e "s|^|sed -i '/|" -e "s|\$|/d'|"  $PRFLFLT
...
if [ ! -s $diffFileTwo ]; then
	...
	d=$( grep -c ^ $diffFileTwo) #echo $BRAND >> $diffFileTwo
...
if [ "$inputSED" == "true" ]; then # Generate custom sed filters for user
	sed -i -e '$d' -e 's![^/]*$!!' -e 's/\/$//' $RECENT
	if [[ "$inputTIME" == "true" ]]; then awk '{print $3}' $RECENT > $TMPCOMPLETE ; else awk '{print $1}' $RECENT > $TMPCOMPLETE ; fi
	sort $TMPCOMPLETE | uniq -c | sort -nr | awk '{print $2}' > $RECENT ; fixcmp < "$RECENT" | fixOT > "$PRFLFLT"
	sed -i -e "s|^|sed -i '/|" -e "s|\$|\\/d'|"  $PRFLFLT
	{ echo "Note these are sorted by highest count first" ; echo $BRAND; echo ; } >> $PRFLFLT
...
if [ "$FLTCHG" == "true" ]; then cyan " Success please review your xCustomerFilterProfile" ; cyan "     in    "$USRDIR$CONFIGDIR"/xCustomFilterProfile" ; cyan " change in deltas detected" ; else cyan " Filter change boolean indicates you dont need to  change" ; cyan "your filter.  ie less than 4 files made it past the filter" ; echo; fi
if  [ "$SYSCHG" == "true" ]; then green " Warn system changed from earlier ensure sterile enviro	" ; else cyan " System didnt change during the search" ; echo ; fi
if [ "$validRLT" == "true" ]; then cyan " Success no files made it past the filters" ; cyan "during the "$deltaDIFF" second search. The delta" ; fi
