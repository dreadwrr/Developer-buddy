#!/usr/bin/env bash
# Merge changes modules _uid_xxxx to L_uid_xxxx with deletions applied                          04/06/2026
...
is_moved="false"							;	is_routine="false"

f=$( ls $PWD | grep -c '.*L_uid_.*.xzm')
[[ "$f" -gt 1 ]] && echo "Cannot have more than one L_uid_.*\.xzm file" && exit 0
...
r=$(ls | grep '.*_uid_.*\.xzm' | grep -v 'L_uid_' | wc -l)
if [ "$r" -gt 0 ]; then
	resolve_conflict "L_uid_" "*.xzm" ".xzm" ".bak" $is_moved $oMF  # cds into /tmp ($output) if moved
	...
	unpack $mtmp $QEXCL $oMF $UPDATE $elog  # cds to changes $ch
	aend=$(date +%s.%N)
	...
	rname="${MODULENM}${SERIAL}L_uid_${ssbn}$$.xzm"
	package_xzm $mtmp $keepMRGED $elog $is_routine "false" $output
