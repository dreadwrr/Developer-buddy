...
if [ "$f" -gt 1 ]; then echo "Cannot have more than one _uid_L\.\*\.xzm file" && exit 0 ; fi
...
if [ "$r" -gt 0 ]; then
    ...
    if [ "$keepMRGED" == "true" ]; then
        while IFS= read -r ofile; do [[ -z "$ofile" || "$ofile" == \#* ]] && continue ; fname=${ofile%.xzm}".bak" ; mv "$ofile" "$fname" ; done < "$oMF"
        unset IFS
    fi
    ...
    if [ $? -ne 0 ]; then
        ...
        rm $oMF ; rm -rf $mtmp ; exit 1
    fi
    if [ "$keepMRGED" == "false" ]; then
        while IFS= read -r ofile; do [[ -z "$ofile" || "$ofile" == \#* ]] && continue ; test -f $ofile && rm $ofile ; done < "$oMF"
        unset IFS
    fi
